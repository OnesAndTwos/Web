...
using OnesAndTwos.Infrastructure.Common;
...
namespace OnesAndTwos.Web.NavigationRoutes
{
    public static class NavigationRouteConfigurationExtensions
    {
        ...
        public static NamedRoute ToDefaultAction<T>(this NamedRoute route, Expression<Func<T, ActionResult>> action,string areaName) where T : IController
        {
            ...
            Guard.Against<ArgumentException>(body == null, "Expression must be a method call");
            Guard.Against<ArgumentException>(body.Object != action.Parameters[0], "Method call must target lambda argument");
        ...
        public static string CreateUrl(string actionName, string controllerName)
        {
            ...
            {
                return actionName.Equals("index", StringComparison.CurrentCultureIgnoreCase) ? "" : actionName.ToLower();
            }
